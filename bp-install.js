...
function isBundleRoute(){return location.hash.startsWith('#/patient/bundle/')}
...
function appName(){if(isBundleRoute())return 'My Care Tools';if(location.hash.startsWith('#/patient/vitamin-d/'))return 'Vitamin D Schedule';if(location.hash.startsWith('#/patient/iron-oral/'))return 'Oral Iron Schedule';if(location.hash.startsWith('#/patient/iron-iv/'))return 'IV Iron Schedule';return 'Blood Pressure Chart'}
function shortName(){if(isBundleRoute())return 'Care Tools';if(location.hash.startsWith('#/patient/vitamin-d/'))return 'Vitamin D';if(location.hash.startsWith('#/patient/iron-oral/'))return 'Oral Iron';if(location.hash.startsWith('#/patient/iron-iv/'))return 'IV Iron';return 'BP Chart'}
...
function installManifest(){
  ...
  const manifest={name:appName(),short_name:shortName(),description:'Patient schedule and tracking tool.',id:current,start_url:current,scope:'./',display:'standalone',orientation:'any',background_color:'#f7fbff',theme_color:'#1767c2',lang:isArabic()?'ar':'en',dir:isArabic()?'rtl':'ltr',icons:[{src:'bp-icon-192.png',sizes:'192x192',type:'image/png',purpose:'any'},{src:'bp-icon-512.png',sizes:'512x512',type:'image/png',purpose:'any maskable'}]};
  if(manifestUrl)URL.revokeObjectURL(manifestUrl);
  manifestUrl=URL.createObjectURL(new Blob([JSON.stringify(manifest)],{type:'application/manifest+json'}));
  let link=document.head.querySelector('link[data-patient-manifest]');if(!link){link=document.createElement('link');link.rel='manifest';link.dataset.patientManifest='1';document.head.appendChild(link)}link.href=manifestUrl;
}
function ensureInstallMetadata(){
  if(!isPatientRoute())return;
  installManifest();
  let icon=document.head.querySelector('link[data-patient-touch-icon]');if(!icon){icon=document.createElement('link');icon.rel='apple-touch-icon';icon.dataset.patientTouchIcon='1';document.head.appendChild(icon)}icon.href='bp-icon-180.png?v=20260722-1';
  ensureMeta('theme-color','#1767c2');ensureMeta('apple-mobile-web-app-capable','yes');ensureMeta('apple-mobile-web-app-status-bar-style','default');ensureMeta('apple-mobile-web-app-title',shortName());
}
...
function targetActions(){return document.querySelector('.vitd-save-actions')||document.querySelector('.bp-tool .bp-controls')}
function removeOldHomeInstructions(){document.querySelectorAll('.vitd-save-card details').forEach(details=>{const label=details.querySelector('summary')?.textContent?.toLowerCase()||'';if(label.includes('home screen')||label.includes('الشاشة الرئيسية'))details.remove()})}
function enhancePatientPage(){
  if(!isPatientRoute()){removeHelp();document.body.classList.remove('bp-patient-active');return}
  ensureInstallMetadata();registerServiceWorker();removeOldHomeInstructions();
  const bpPage=isBpRoute()||!!document.querySelector('.bp-tool');
  if(bpPage){document.body.classList.add('bp-patient-active');document.querySelector('.bp-head .detail-back')?.remove();document.querySelectorAll('.bp-tool a[href]').forEach(link=>link.remove())}else document.body.classList.remove('bp-patient-active');
  const controls=targetActions();if(!controls)return;
  const existing=controls.querySelector('[data-patient-install-home]');if(isStandalone()){existing?.remove();return}if(existing)existing.innerHTML=buttonLabel();else controls.prepend(installButton());
}
