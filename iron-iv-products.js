...
const PRODUCTS={
  carboxymaltose:{
    label:'Ferric carboxymaltose',concentration:50,unitMg:500,type:'calculated',gap:7,
    recommendStep:100,doseStep:100,
    note:'50 mg/mL. Maximum scheduled dose is the lower of 20 mg/kg or 1,000 mg, with additional doses at least 7 days apart.'
  },
  derisomaltose:{
    label:'Ferric derisomaltose',concentration:100,unitMg:1000,type:'calculated',gap:7,
    recommendStep:100,doseStep:100,
    note:'100 mg/mL. For 50 kg or more, the preset maximum is 1,000 mg in one infusion; below 50 kg, 20 mg/kg.'
  },
  ferumoxytol:{
    label:'Ferumoxytol',concentration:30,unitMg:510,type:'ferumoxytol',gap:3,
    recommendStep:510,doseStep:510,
    note:'510 mg per administration. When two administrations are prescribed, the second is scheduled 3 days later; the labeled interval is 3–8 days.'
  },
  sucrose:{
    label:'Iron sucrose',concentration:20,unitMg:100,type:'calculated',gap:2,maxDose:200,
    recommendStep:100,doseStep:50,
    note:'20 mg/mL. This general-adult local preset uses a clinician-confirmed maximum of 200 mg per visit and a 2-day interval; it is not a CKD dosing preset.'
  },
  custom:{
    label:'Custom IV iron formulation',concentration:20,unitMg:100,type:'custom',gap:7,maxDose:200,
    recommendStep:100,doseStep:50,
    note:'Enter the local concentration, iron per ampoule or vial, maximum per visit, dose increment, and interval.'
  }
};
...
function recommendation(){
  ...
  if(branch()==='ida'){
    ...
    return {
      ...
      low:null,
      high:null,
      suggested:roundNearest(raw,p.recommendStep||100),
      weight,
      basis:`Ganzoni: ${weight} × (${target} − ${hb}) × 2.4 + ${stores}`
    };
  }
  ...
  return {
    ...
    weight,
    ferritin:current,
    targetFerritin:target,
    basis:`Ferritin gap ${delta} µg/L ÷ 0.21 µg/L per mg elemental iron`
  };
}
...
function splitSchedule(total,p,weight,start){
  ...
  if(productId()==='ferumoxytol'){
    ...
    return {doses:Array.from({length:count},()=>510),offsets:Array.from({length:count},(_,i)=>i*3),maxDose:510};
  }
  ...
  while(left>0){const dose=Math.min(left,usableMax);doses.push(dose);offsets.push(i*p.gap);left-=dose;i++}
...
function schedule(){
  ...
  const prescribed=n('ivp-prescribed');
  if(!prescribed)return null;
  const split=splitSchedule(prescribed,p,rec.weight,start);
  if(split.error)return split;
  const infusions=split.doses.map((mg,i)=>({
    date:add(start,split.offsets[i]),
    mg,
    ml:Number((mg/p.concentration).toFixed(2)),
    vials:Number((mg/p.unitMg).toFixed(2))
  }));
  return {
    ...
    dp:Math.max(...split.doses),mx:split.maxDose,g:p.gap,s:start,r:repeat,i:infusions,c:clinic,
    summary:rec.basis,goal:branch()==='no-ida'?rec.targetFerritin:null,preset:productId(),productNote:p.note
  };
}
...
function refresh(){
  ...
  document.getElementById('ivp-note').innerHTML=`<strong>${e(p.label)}</strong><p>${e(String(p.concentration||'—'))} mg/mL · ${e(String(p.unitMg||'—'))} mg per ampoule/vial. ${e(p.note)}</p>`;
  if(rec){
    document.getElementById('ivp-recommended').value=String(rec.suggested);
  }
  ...
  const estimate=branch()==='ida'
    ?`Calculated requirement: ${rec.raw.toFixed(1)} mg. Recommended review amount: ${rec.suggested} mg.`
    :`Research estimate: ${rec.raw.toFixed(1)} mg (approximate range ${rec.low.toFixed(1)}–${rec.high.toFixed(1)} mg). Recommended review amount: ${rec.suggested} mg.`;
  if(!x){out.innerHTML=`<strong>${estimate}</strong><span>Enter the clinician-prescribed elemental iron to build the formulation schedule.</span>`;return}
  ...
  out.innerHTML=`<strong>${estimate}</strong><span>Clinician prescribed: ${x.t} mg. ${x.p}: ${x.i.length} visit${x.i.length===1?'':'s'} — ${x.i.map((a,i)=>`Visit ${i+1}: ${a.mg} mg (${a.ml} mL) on ${nice(a.date)}`).join(' · ')}</span>`;
}
...
function exportPlan(x){
  ...
  const estimate=x.branch==='ida'
    ?`${x.required.toLocaleString()} mg calculated by Ganzoni`
    :`${x.required.toLocaleString()} mg research estimate${x.estimateLow!=null?` (${x.estimateLow.toLocaleString()}–${x.estimateHigh.toLocaleString()} mg range)`:''}`;
  target.innerHTML=`<div class="vitd-export-head"><div><p class="eyebrow">PATIENT HANDOFF</p><h2>${e(x.p)} schedule</h2><p>The clinician-prescribed amount—not the research estimate—controls this schedule.</p></div><div id="ivp-qr" class="vitd-qr"></div></div><div class="vitd-export-summary"><article><small>Calculation</small><strong>${e(estimate)}</strong><span>Recommended review amount ${x.suggested} mg</span></article><article><small>Clinician prescribed</small><strong>${x.t.toLocaleString()} mg</strong><span>${e(x.p)} · ${x.concentration} mg/mL</span></article><article><small>Appointments</small><strong>${x.i.length}</strong><span>Repeat labs ${nice(x.r)}</span></article></div><div class="iv-appointment-list">${x.i.map((a,i)=>`<article><strong>Infusion ${i+1}</strong><span>${nice(a.date)} · ${a.mg} mg · ${a.ml} mL · ${a.vials} vial equivalent</span></article>`).join('')}</div><label class="vitd-link-label">Patient link<input id="ivp-link" readonly value="${e(patient)}"></label><div class="vitd-export-actions"><button class="btn dr" data-ivp-open>Open patient page</button><button class="btn ghost" data-ivp-copy>Copy link</button></div>`;
...
document.addEventListener('change',ev=>{
  if(ev.target.name==='iv-product-branch'){
    const ida=branch()==='ida';
    syncWeightFields(ida?'ida':'no-ida');
    document.getElementById('ivp-ida').hidden=!ida;
    document.getElementById('ivp-noida').hidden=ida;
    syncSuggested(true);
...
document.addEventListener('click',async ev=>{
  if(ev.target.closest('[data-use-iv-recommendation]')){syncSuggested(true);refresh();return}
